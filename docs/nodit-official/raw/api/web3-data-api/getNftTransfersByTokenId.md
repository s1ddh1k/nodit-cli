...
Retrieves the list of transfers for a specific NFT. Results include contract metadata and NFT metadata.
...
| tokenId | string | ✓ | Parameter specifying the NFT token ID to query. Can be entered as a decimal string. |
...
| withFunction | boolean |  | Parameter specifying whether to include the function field in the response. Response speed may be slower when set to true. |
...
| items[].nft | object |  |  |
| items[].nft.tokenId | string | ✓ | A field representing the unique ID of the NFT token. This ID is used to identify the NFT. |
| items[].nft.tokenUri | string | ✓ | A field representing the URI where the original metadata of the NFT is located. Format may vary by contract implementation, but is typically provided as an IPFS address or web URL. |
| items[].nft.tokenUriSyncedAt | string | ✓ | A field representing when the tokenUri of the NFT was synchronized. |
| items[].nft.rawMetadata | string |  | A field representing the rawMetadata of the NFT. Included in the response only when the withMetadata parameter is set to true. |
| items[].nft.metadataSyncedAt | string |  | A field representing when the metadata of the NFT was synchronized. Included in the response only when the withMetadata parameter is set to true. |
