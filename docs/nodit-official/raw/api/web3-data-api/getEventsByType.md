...
Retrieves the list of events filtered by a specific event type.
...
|------|------|----------|-------------|
| eventType | string | ✓ | Parameter specifying the event type to query. Event type refers to the name of the event struct defined in the module. This field must be entered in the format `module_address::module_name::event_name`. |
