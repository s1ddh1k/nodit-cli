...
Retrieves sequence-number-based transactions committed on-chain by an account. (Transactions without a sequence number are not retrieved.)
If the start version is too far in the past, a 410 error is returned. If no start version is specified, retrieval starts from version 0.
