...
| jsonrpc | string | ✓ |  |
| method | string | ✓ |  |
| params | array | ✓ | Enter the following parameters as an array with the appropriate types.  1. `block hash`: Enter the block hash to query in 64-digit hexadecimal string format. 2. `traceOption`: Object for trace option configuration. |
