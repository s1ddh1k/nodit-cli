...
| jsonrpc | string | ✓ |  |
| method | string | ✓ |  |
| params | array | ✓ | Pass the following parameters as an array with the appropriate types. 1. `block number or block tag`: Block number as hex string, or block tag such as "earliest", "latest", "pending". 2. `transaction index`: The transaction index as a hex string. |
